...
def batch_solve_affine(
    self, 
    source_batches: np.ndarray,
    target_batches: np.ndarray,
    method: str = 'least_squares'
) -> np.ndarray:
    """
    Solve multiple affine transformations simultaneously using vectorized operations.
    
    Args:
        source_batches: Array of shape (n_batches, n_points, dimension)
        target_batches: Array of shape (n_batches, n_points, dimension)
        method: 'least_squares' or 'pinv' (pseudo-inverse)
        
    Returns:
        Transformation matrices of shape (n_batches, dimension+1, dimension+1)
    """
    source_batches = np.asarray(source_batches)
    target_batches = np.asarray(target_batches)
    
    if source_batches.shape != target_batches.shape:
        raise ValueError("Source and target batches must have same shape")
        
    n_batches, n_points, dim = source_batches.shape
    
    if dim != self.dimension:
        raise ValueError(f"Points must be {self.dimension}D")
        
    if n_points < self.dimension + 1:
        raise ValueError(f"Need at least {self.dimension + 1} points per batch")
    
    # Create homogeneous coordinates for all batches
    # Shape: (n_batches, n_points, dimension+1)
    ones = np.ones((n_batches, n_points, 1))
    source_homo = np.concatenate([source_batches, ones], axis=2)
    
    if method == 'least_squares':
        # Use normal equations: (A^T A) x = A^T b
        # For each batch: source_homo.T @ source_homo @ x = source_homo.T @ target
        
        # Compute A^T A for all batches: (n_batches, dim+1, dim+1)
        ATA = np.matmul(source_homo.transpose(0, 2, 1), source_homo)
        
        # Compute A^T b for all batches: (n_batches, dim+1, dimension)
        ATb = np.matmul(source_homo.transpose(0, 2, 1), target_batches)
        
        # Solve all systems simultaneously
        try:
            # Shape: (n_batches, dim+1, dimension)
            coeffs = np.linalg.solve(ATA, ATb)
        except np.linalg.LinAlgError:
            # Fallback to pseudo-inverse if singular
            coeffs = np.matmul(np.linalg.pinv(ATA), ATb)
            
    elif method == 'pinv':
        # Direct pseudo-inverse approach
        # Shape: (n_batches, dim+1, n_points)
        source_pinv = np.linalg.pinv(source_homo)
        
        # Shape: (n_batches, dim+1, dimension)
        coeffs = np.matmul(source_pinv, target_batches)
    else:
        raise ValueError("Method must be 'least_squares' or 'pinv'")
    
    # Construct transformation matrices
    # Shape: (n_batches, dimension+1, dimension+1)
    transform_matrices = np.zeros((n_batches, self.dimension + 1, self.dimension + 1))
    
    # Fill in the linear transformation and translation parts
    transform_matrices[:, :self.dimension, :] = coeffs.transpose(0, 2, 1)
    
    # Set homogeneous coordinate
    transform_matrices[:, self.dimension, self.dimension] = 1.0
